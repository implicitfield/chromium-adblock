...
#include "chrome/browser/brave_shields/ad_block_pref_service_factory.h"
...
#include "components/brave_shields/content/browser/ad_block_pref_service.h"
#include "components/brave_shields/content/browser/ad_block_service.h"
#include "components/brave_shields/core/common/brave_shield_constants.h"
...
void OnShouldBlockRequestResult(bool then_check_uncloaked,
  ...
  if (ctx->blocked_by() != kAdBlocked && then_check_uncloaked) {
...
void UseCnameResult(const ResponseCallback& next_callback,
  ...
  if (cname.has_value() && ctx->request_url().host() != *cname &&
    ...
    ShouldBlockRequestParams input{
        ...
        ctx->resource_type(),      ctx->method(),
        ctx->render_frame_token(), ctx->devtools_request_id()};
    auto* ad_block_service = g_browser_process->ad_block_service();
...
int OnBeforeURLRequest_AdBlockTPPreWork(const ResponseCallback& next_callback,
  ...
  if (ctx->initiator_url().SchemeIs(extensions::kExtensionScheme) &&
      !base::FeatureList::IsEnabled(
