...
#include "components/cosmetic_filters/renderer/cosmetic_filters_js_handler.h"
...
void CosmeticFiltersJSHandler::ApplyRules(bool de_amp_enabled) {
  ...
  std::string cosmetic_filtering_init_script = absl::StrFormat(
      kCosmeticFilteringInitScript, generichide_ ? "true" : "false",
...
void CosmeticFiltersJSHandler::CSSRulesRoutine(
  ...
  const auto* hide_selectors_list = resources_dict.FindList("hide_selectors");
